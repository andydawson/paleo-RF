...
REVEALS outputs); pointer to `DATA.md`, `CHANGELOG.md`, `KNOWN_ISSUES.md`.
...
| 3 | Delete the non-interp/point code, `archive/`, `thornthwaite.R`, `GCM_snow_prob.R`, `beta_veg_lct_modern.R`, all `# XXX OLD` blocks and every commented-out block; drop the legacy data files (KI#31) after the datasheet records them; tag `v0.3-interp-only`. | Half the line count gone; one flavour; `CODE_MAP.md` shrinks to a page. | 1 day |
